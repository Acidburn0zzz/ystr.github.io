...
char* NextProcStatColumn (char* start, unsigned long* out)
{
	...
	{
		if (*next == ' ' || *next == '\n') *next = '\0';
		if (*next == '\0') { next++; sscanf(start, "%lu", out); return next; }
	}
}
...
void GetProcStat (ProcStat* ps)
{
	...
	static char raw[1024]; char* r = fgets(raw, sizeof(raw), f);
...
char* GetRam ()
{
	...
	sprintf(ram, "%luM", (pmi.MemTotal - pmi.MemFree - pmi.Cached - pmi.Buffers) / 1024);
	return ram;
}
